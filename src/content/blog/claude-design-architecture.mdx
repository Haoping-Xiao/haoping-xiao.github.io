...
**渲染的本质**始终是：JS 解析模板或组件描述，操作 DOM，浏览器完成绘制。区别只在于「模板长什么样」——标准 HTML、React JSX，还是下面要说的自定义 DSL。
...
简历和 HTML 组件走一套内部自定义 DSL，浏览器原生不解析这些标签，只有对应渲染引擎认识。它的能力包括：循环渲染、数据插值、向 `<head>` 注入样式等——本质上是在标准 Web 平台上叠一层受控语法，让模型输出既结构化又比纯 JS 更稳定。

这和 Vue 的 `v-for`、React 的 JSX 是同一类思路：不是让浏览器理解新语法，而是让运行时在执行时把它翻译成合法的 DOM 操作。

PPT 则走 Web Component 路线，与简历渲染器完全独立——说明 Claude Design 没有强行用一种渲染器覆盖所有类型，而是**按产物形态选最省事的解释路径**。
