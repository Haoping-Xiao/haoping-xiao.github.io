...
[Workload Identity Federation（WIF）](https://platform.claude.com/docs/en/manage-claude/workload-identity-federation) 的答案和 [Vercel Connect](/blog/vercel-connect-deep-dive) 表面相似——**不存密钥，按需换取**——但方向恰好相反。我最初把方向搞反了，也以为只有一个 Token，还和出站凭证代理混为一谈。下面按我踩过的三个盲区展开。
...
在看换票流程之前，先认识 Claude Console 里要配的三个资源，合起来表达一句话：**「Issuer X 签发的、满足条件 Y 的 JWT，可以冒充 Service Account Z 行动。」**
...
实际上是**两次交换、三张票、三步走完**（第三步是拿 Claude 发的票去调 API，不是第三次换票）：

```mermaid
sequenceDiagram
    participant WL as Workload<br/>(GitHub CI / Lambda / Pod)
    participant IDP as Issuer / IdP<br/>(GitHub / AWS / Okta)
    participant TOKEN as POST /v1/oauth/token<br/>(Claude Token Exchange)
    participant CP as Claude Platform<br/>(Federation Rule / Service Account)
    participant API as Claude API

    Note over WL,IDP: 第一步：获取外部身份证明
    WL->>IDP: 请求 OIDC Token（由平台自动注入，无需代码请求）
    IDP-->>WL: 返回签名 JWT<br/>包含 iss / sub / aud 等 Claims

    Note over WL,CP: 第二步：用外部身份换 Claude 凭证
    WL->>TOKEN: 提交 JWT + federation_rule_id<br/>（RFC 7523 jwt-bearer grant）
    TOKEN->>CP: 验证 JWT 签名（JWKS）<br/>匹配 Federation Rule 的 Claims 条件
    CP-->>TOKEN: 验证通过，映射到 Service Account
    TOKEN-->>WL: 返回短期 Claude Access Token<br/>sk-ant-oat01-...（默认 3600s）

    Note over WL,API: 第三步：用 Claude Token 调用 API
    WL->>API: 携带 sk-ant-oat01-... 调用 Claude API
    API-->>WL: 正常响应（受 Service Account 的 Role/Scope 约束）

    Note over WL,TOKEN: SDK 自动刷新：到期前 120s 后台刷新 / 30s 前强制刷新
```

1. **IdP 发 OIDC JWT** — 证明「这个 workload 来自 `repo:acme/app:ref:refs/heads/main`」或「这个 Pod 是 `prod:worker`」。这张票**不能直接调 Claude**。
2. **Claude 验票后发 `sk-ant-oat01-...`** — 通过 RFC 7523 `jwt-bearer` grant 交换，绑定到你配置的 Service Account，寿命默认 3600 秒（Rule 可配 60–86400）。SDK 会在到期前 120s 尝试后台刷新（失败不报错），30s 前强制刷新（失败则报错）。
3. **拿 Access Token 调 API** — `Authorization: Bearer` 发请求，权限受 Rule 授予的 scope 约束，调用记录归因到对应 Service Account。

底层 HTTP 长这样（摘自[官方文档](https://platform.claude.com/docs/en/manage-claude/workload-identity-federation)）：

```bash
# 1. 从平台拿到 IdP JWT（GitHub Actions 自动注入，K8s 通过 ServiceAccount projected volume 挂载）
...
2. **Rule 的粒度才是安全增量** — 联邦机制解决的是「密钥不用分发」；真正拉开与共享 API Key 差距的，是 Rule 把「哪个 workload 能冒充哪个 Service Account、拿什么 scope」写死在 Claims 上。如果 Rule 只匹配到 org 级别或整个仓库，你实际上只是把静态 API Key 换成了一种按需签发的万能钥匙，安全增量接近于零。
3. **事故排查要串两张票的日志** — IdP 侧看谁签发了哪条 `sub`（GitHub Actions audit、CloudTrail），Claude 侧看哪个 Service Account 换了票、打了哪些 API。一边断链，归因就退回到「某把 key 不知被谁用了」。
