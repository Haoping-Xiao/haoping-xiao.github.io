...
sequenceDiagram
    ...
    WL->>IDP: 请求 OIDC Token（ambient，平台自动注入）
    ...
    Note over WL,TOKEN: SDK 自动刷新：到期前 120s advisory / 30s mandatory
```

1. **IdP 发 OIDC JWT** — 证明「这个 workload 来自 `repo:acme/app:ref:refs/heads/main`」或「这个 Pod 是 `prod:worker`」。这张票**不能直接调 Claude**。
2. **Claude 验票后发 `sk-ant-oat01-...`** — 通过 RFC 7523 `jwt-bearer` grant 交换，绑定到你配置的 Service Account，寿命默认 3600 秒（Rule 可配 60–86400）。SDK 会在到期前 120s 尝试 advisory 刷新、30s 前 mandatory 刷新。
...
2. **Rule 的粒度才是安全增量** — 联邦机制解决的是「密钥不用分发」；真正拉开与共享 API Key 差距的，是 Rule 把「哪个 workload 能冒充哪个 Service Account、拿什么 scope」写死在 Claims 上。Rule 只匹配到 org 或整仓，你只是换了一种形态的万能钥匙。
